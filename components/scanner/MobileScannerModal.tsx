...
import { Camera, VideoOff, Barcode, Volume2, ShieldCheck, X, Copy, Check } from 'lucide-react';
  ...
  const startCamera = async () => {
    try {
      if (!scannerRef.current) {
        // EXCLUSIVO PARA CÓDIGOS DE BARRAS 1D DE ENVÍO Y LOGÍSTICA (SIN QR)
      ...
      const config = {
        fps: 30, // Máxima velocidad 30 FPS dedicada a barras 1D
        qrbox: (vfWidth: number, vfHeight: number) => {
          return {
            width: Math.floor(vfWidth * 0.92),
            height: Math.floor(vfHeight * 0.5) // Ratio alargado especial para barras lineales
          };
        },
      ...
      console.log('Error start camera mobile:', err);
    }
  };
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: '#020617', borderBottom: '1px solid #1e293b' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <h3 style={{ fontSize: '13.5px', fontWeight: 800, margin: 0, color: '#ffffff' }}>Escáner Exclusivo Código de Barras (CODE 128)</h3>
              <p style={{ fontSize: '10px', color: '#38bdf8', fontFamily: 'JetBrains Mono', margin: 0 }}>Especializado en Guías & Rótulos AMEX (Sin QR)</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div style={{ padding: '16px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ position: 'relative', width: '100%', backgroundColor: '#020617', borderRadius: '12px', overflow: 'hidden', minHeight: '240px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #1e293b' }}>
            ...
            {isScanning && (
              <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '16px' }}>
                <div style={{ width: '100%', height: '3px', background: 'linear-gradient(to right, transparent, #ef4444, #38bdf8, #ef4444, transparent)', boxShadow: '0 0 14px #ef4444' }}></div>
                <div style={{ textAlign: 'center', fontSize: '10px', fontWeight: 700, color: '#7dd3fc', background: 'rgba(2, 6, 23, 0.85)', padding: '6px 12px', borderRadius: '6px', border: '1px solid rgba(56,189,248,0.3)' }}>
                  Alinee las barras horizontales `AMX...` en la guía
                </div>
              </div>
            )}
            ...
            {!isScanning && (
              <div style={{ textAlign: 'center', padding: '24px', color: '#94a3b8' }}>
                ...
                <p style={{ fontWeight: 700, color: '#ffffff', fontSize: '13px', margin: 0 }}>Cámara en espera</p>
                <p style={{ fontSize: '11px', color: '#94a3b8', marginTop: '4px' }}>
                  Toca el botón para iniciar la detección exclusiva de código de barras
                </p>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {!isScanning ? (
              ...
              >
                <Camera className="w-4 h-4" /> Activar Lector de Barras 1D
              </button>
            ) : (
              <button
                onClick={stopCamera}
          ...
          {scanHistory.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>Códigos de Barras Detectados</span>
